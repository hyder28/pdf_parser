...
if __name__ == "__main__":
    ...
    f_dir = "./data/"
    output_fdir = f_dir + "_pdf-extractions_{}/".format(datetime.now().strftime('%d-%m-%Y_%H%M'))


    if not os.path.exists(output_fdir):
        print("Creating file directory at --> {}\n".format(output_fdir))
    ...
    for index, pdf_file in enumerate(pdf_files):
        ...
        if is_dpi_valid == False:
            print(f"The pdf DPI is less than 250 for pages {faulty_page_nos} of {pdf_file}")
        ...
        for page in doc:
            if page_details[page.number]["drm_status"] == False:
                page_data = page_proc.extract_data_from_page(f_dir + pdf_file, page, page_details[page.number]["is_scanned"])
        ...
        pdf_data.to_csv(output_fdir + f"{pdf_file[:-4]}-pdf-process-data.csv")
        ...
        df = pc_relation_gen.generate_relationship(pdf_data, pdf_file)

        df.to_csv(output_fdir + f"{pdf_file[:-4]}-parent-child-relation.csv")

    elapsed_time = datetime.now() - start_time
    print("----- Total execution time: {} min {} sec -----".format(int(divmod(elapsed_time.total_seconds(), 60)[0]), int(divmod(elapsed_time.total_seconds(), 60)[1])))
